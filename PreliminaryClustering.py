...
class PreliminaryClustering:
    ...
    def __scale_features(self, velocities):
        ...
        features_train_frames = np.array(
            [feature_frame for feature_video in train_velocities for feature_frame in feature_video])
        features_all_frames = np.array(
            [feature_frame for feature_video in velocities for feature_frame in feature_video])
    ...
    def __plot_and_save_histograms(self, histo_figures_path):
        ...
        for idx, histo in enumerate(self.histograms_of_videos):
            if len(self.index_neutral_configurations):
                plt.bar(self.index_neutral_configurations, histo[np.array(self.index_neutral_configurations)],
                        color="blue")
            if len(self.index_relevant_configurations):
                plt.bar(self.index_relevant_configurations, histo[np.array(self.index_relevant_configurations)],
                        color="red")
