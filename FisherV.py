...
class FisherVectorV:
    ...
    def fit_by_bic(self, X, choices_n_kernels=N_Kernel_Choices, model_dump_path=None, verbose=True):
        ...
        if X.ndim == 4:
            self.ndim = 4
            return self._fit_by_bic(X, choices_n_kernels=choices_n_kernels, model_dump_path=model_dump_path,
                                    verbose=verbose)
    ...
    def _fit(self, X, model_dump_path=None, verbose=True):
        ...
        self.gmm = gmm = GaussianMixture(n_components=self.n_kernels, covariance_type=self.covariance_type,
                                         max_iter=1000).fit(X)
    ...
    def compute_likelihood_ratio(self, X, normalized=True):
        """
        Computes likelihood ratio q_ik
